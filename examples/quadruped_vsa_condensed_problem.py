...
class SimpleQuadrupedalGaitProblem:
    def __init__(self, rmodel, lfFoot, rfFoot, lhFoot, rhFoot):
        ...
        self.actuation = aslr_to.FreeFloatingActuationCondensed(self.state,3*12)
    ...
    def createSwingFootModel(self, timeStep, supportFootIds, comTask=None, swingFootTask=None):
        ...
        uActivation = crocoddyl.ActivationModelWeightedQuad(np.array([1e0]*12+[1e1]*12 + [0] * 12 ))
        uResidual = crocoddyl.ResidualModelControl(self.state, nu)
        ctrlReg = crocoddyl.CostModelResidual(self.state, uActivation,uResidual)
        ...
        feas_residual = aslr_to.FloatingVSADynamicsResidualModel(self.state, nu)
        ...
        u_lb = np.concatenate([-100*np.ones(24),1*np.ones(12)])
        u_ub = np.concatenate([100*np.ones(24),100*np.ones(12)])
        controlBoundsResidual = crocoddyl.ResidualModelControl(self.state,nu)
        controlBoundActivation = crocoddyl.ActivationModelQuadraticBarrier(crocoddyl.ActivationBounds(u_lb, u_ub))
        controlBounds = crocoddyl.CostModelResidual(self.state, controlBoundActivation, controlBoundsResidual)
        costModel.addCost("controlBounds", controlBounds, 1e9)
