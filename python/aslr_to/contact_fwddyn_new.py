...
class DifferentialContactASLRFwdDynModelNew(crocoddyl.DifferentialActionModelAbstract):
    def __init__(self, state, actuation, contacts, costs, K=None, B=None):
        nu =  actuation.nu + state.nv_m
        nh = state.nv_m 
        crocoddyl.DifferentialActionModelAbstract.__init__(self, state, nu, costs.nr, 0, nh)
        self.constraints = crocoddyl.ConstraintModelManager(self.state, nu)
        ...
        self.constraints.addConstraint(
            "rnea",
            crocoddyl.ConstraintModelResidual(state, aslr_to.ResidualModelTauCouple(state, nu, self.K)))
    ...
    def calc(self, data, x, u):
        ...
        tau_k = data.multibody.actuation.tau

        JMinvJt_damping_=0.
        pinocchio.forwardDynamics(self.state.pinocchio, data.multibody.pinocchio, data.multibody.actuation.tau, data.multibody.contacts.Jc[:nc,:nv_l],
                        data.multibody.contacts.a0, JMinvJt_damping_)
        data.xout[:nv_l] = data.multibody.pinocchio.ddq
        data.xout[nv_l:] = np.dot(data.Binv, u[self.state.nv_m:] + u[:self.state.nv_m])
    ...
    def calcDiff(self, data, x, u):
        ...
        data.Fx[:nv_l,:nv_l] = -np.dot(a_partial_dtau, data.multibody.pinocchio.dtau_dq)
        ...
        data.Fu[:nv_l, :nv_m] =  np.dot(a_partial_dtau,data.multibody.actuation.dtau_du[:nv_l, :])
        data.Fu[nv_l:, :nv_m] =  np.linalg.inv(self.B)
        data.Fu[nv_l:, nv_m:] =  np.linalg.inv(self.B)

        #computing the jacobian of contact forces (required with contact dependent costs)
        data.df_dx =np.zeros([self.contacts.nc,self.state.ndx])
        ...
        data.df_du[:nc, :nv_m] = -np.dot(f_partial_dtau, data.multibody.actuation.dtau_du[:nv_l,:])
    ...
    def createData(self):
        data = DifferentialContactASLRFwdDynDataNew(self)
        return data

class DifferentialContactASLRFwdDynDataNew(crocoddyl.DifferentialActionDataAbstract):
    def __init__(self, model):
        ...
        self.constraints = model.constraints.createData(self.multibody)
        self.costs.shareMemory(self)
        self.constraints.shareMemory(self)
        self.Minv = None
        self.Kinv = None

        #contact jacobians
        self.df_dx =np.zeros([model.contacts.nc,model.state.ndx])
        self.df_du =np.zeros([model.contacts.nc, model.actuation.nu+model.state.nv_m])
