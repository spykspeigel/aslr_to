...
class SolverEQ(crocoddyl.SolverAbstract):
    ...
    def computeGains(self, model, data, t):
        try:
            if model.nu > 0 and model.nh > 0:
                ...
                Psi = - self.Qxu[t].T
                Phi = - self.Qu[t] - np.dot(data.Hu.T,self.pi[t]) 
    ...
    def forwardPass(self, stepLength, warning='ignore'):
        ...
        for t, (m, d) in enumerate(zip(self.problem.runningModels, self.problem.runningDatas)):
            ...
            #     self.s[t] += np.dot(np.linalg.inv(Lamda),rs) - np.dot(np.linalg.inv(SinvLamda),del_Lamda)
            #     self.lamda[t] +=  del_Lamda
    ...
    def allocateData(self):
        ...
        self.tau = [np.ones([m.ng]) for m in self.problem.runningModels]
