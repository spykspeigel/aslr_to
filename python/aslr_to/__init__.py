    ...
    def getForceTrajectoryFromSolver(self, solver):
        ...
        for i, data in enumerate(datas):
            ...
            if hasattr(data, "differential"):
                if isinstance(data.differential, contact_fwddyn_aslr.DifferentialContactASLRFwdDynData):
                    fc = []
                    for key, contact in data.differential.multibody.contacts.contacts.todict().items():
                        if model.differential.contacts.contacts[key].active:
                            ...
                            for k, c in model.differential.costs.costs.todict().items():
                                if isinstance(c.cost.residual, crocoddyl.ResidualModelContactFrictionCone):
                    ...
                    fs.append(fc)
                elif isinstance(data.differential, crocoddyl.StdVec_DiffActionData):
                    fc = []
        ...
        for key, p in ps.items():
            ...
            for i, data in enumerate(datas):
                ...
                if hasattr(data, "differential"):
                    ...
                    if isinstance(data.differential, crocoddyl.StdVec_DiffActionData):
...
class CallbackDisplay(crocoddyl.CallbackAbstract):
