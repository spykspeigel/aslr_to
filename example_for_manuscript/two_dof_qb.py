...
uActivation = crocoddyl.ActivationModelWeightedQuad(np.array([1e-1]+[1e-1] + [1e-2] * 2 ))
...
runningModel = aslr_to.IntegratedActionModelEulerASR(
    aslr_to.DifferentialFreeFwdDynamicsModelQb(state, actuation, runningCostModel,B), dt)
terminalModel = aslr_to.IntegratedActionModelEulerASR(
    aslr_to.DifferentialFreeFwdDynamicsModelQb(state, actuation, terminalCostModel,B), 0)

l_lim_0=0
runningModel.u_lb = np.array([ -10, -10, .01, .01])
...
savemat("optimised_trajectory_qb.mat", {"q1": q1,"q2":q2,"qm1": qm1,"qm2":qm2,"v1": v1,"v2":v2,"vm1": vm1,"vm2":vm2,"t":t})
savemat("controls_qb.mat", {"u1": u1,"u2":u2,"t":t})
savemat("stiffness_qb.mat", {"u3": u3,"u4":u4,"t":t})
